...
def plot_ai(csv_path, plot_path, split_label):
    ...
    for ax, measure, label in zip(axes, MEASURES, MEASURE_LABELS):
        ...
        # ── Step 1: Kruskal-Wallis overall test across all 3 groups ──────
        kw_stat, kw_p = kruskal(*data_by_group)

        # ── Step 2: Pairwise Welch's t-test brackets ──────────────────────
        ...
        for i, (idx1, idx2) in enumerate(itertools.combinations(range(len(GROUP_ORDER)), 2)):
            ...
            # Welch's t-test: equal_var=False handles unequal sizes & variances
            _, p    = ttest_ind(g1, g2, equal_var=False, alternative='two-sided')
            x1, x2  = idx1 + 1, idx2 + 1
            y = y_max + step * (i + 1)
        ...
        # Kruskal-Wallis p-value shown in panel title
        ax.set_xticks([1, 2, 3])
        ax.set_xticklabels(
    ...
    plt.suptitle(
        f'Hemispheric Asymmetry Index by Diagnosis — {split_label}\n'
        f'Brackets: Welch\'s t-test  |  Panel header: Kruskal-Wallis (overall)',
